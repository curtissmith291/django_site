...
def eq_analysis_view(request, *args, **kwargs):
    ...
    # Add figure title
    fig.update_layout(
        title_text="Monthly Count of Magnitude >=3 Earthquakes and Monthly Injection Volumes"
    )
    ...
    # Set y-axes titles
    fig.update_yaxes(title_text="Monthly Injection Volume (m3)", secondary_y=False)
    ...
    context = {'graph_1': graph_1, 'graph': graph}
    # context = {'graph': graph}
    # response = render(request, 'ok_eq.html', context)


    return render(request, "eq_analysis.html", context)